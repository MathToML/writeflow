...
- 반복적인 활동 추적: 찬물샤워, 뽀모도로 공부 횟수, 성경 읽기 등
- 각 항목에 **간단한 기록** 가능 (예: "마태복음 1장 읽음")
- **GitHub 스타일 Heatmap**으로 꾸준함 시각화
- 각 셀 클릭 시 해당 날짜의 세부 정보 표시
- Streak 카운터 및 격려 메시지

### 4.5 기록 (Records) — 작업 기억

일을 수행하기 위한 참조 정보를 저장:
...
```
Memory
├── Short-term (오늘의 brain dump, 진행 중인 작업)
├── Long-term (분류된 할 일, 일정, 기록, 완료된 항목)
├── Persona (사용자 프로필, 선호도, 패턴)
└── Archive (날짜별 daily dump 히스토리)
```
...
- **Vertex AI RAG Engine**: 데이터가 대량으로 쌓이면 전환
...
- 검색 시 semantic search로 관련 기록 탐색
...
records
├── id, user_id, dump_id
├── category (contact | procedure | family | shopping | client | ...)
├── title, content (jsonb)
├── tags (text[])
└── created_at, updated_at
...
### Phase 2: 확장 입력 + Proactive

- [ ] 음성 입력 (Speech-to-Text)
- [ ] 이미지 입력 (Gemini multimodal 분석)
...
- [ ] Semantic search (Vertex AI RAG Engine)
